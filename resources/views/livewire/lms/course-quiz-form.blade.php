<form id="kt_account_profile_details_form" class="form" wire:submit.prevent="submit">
  ...
  <div class="page-box-main-fields">
    <div class="row">
      ...
      <div class="col-sm-12 col-md-4 quiz-type-field">
        <x-select-simple-field label="Quiz Type" name="courseQuiz.course_quiz_type_id" required="true"
                               :items="$quizTypes"/>
      </div>
      <div class="col-sm-12 col-md-4">
  ...
  <div class="page-box-uploads">
    ...
    <div class="row">
      ...
      <div class="col">
        <x-file-field name="video" label="Video" :pictureTemp="$video" value="{{$courseQuiz->video}}"
  ...
  @if($courseQuiz->id)
    <div class="page-box-content page-box-content-quiz">
      ...
      <div class="row">
        <div class="col">
          <a href="{{route('admin-page-editor', ['id' => $courseQuiz->staticPage->id])}}"
          ...
          <div id="collapse-question-{{$courseQuiz->id.'-'.$quest->id}}" class="accordion-collapse collapse"
            ...
            <div class="accordion-body">
              <div class="row">
                ...
                <x-text-area-field label="{{__('Question')}} - {{$loop->iteration}}"
                        ...
                        @foreach($quest->alternatives as $alt)
                          <tr>
                            <td>
                              ...
                              <x-text-area-field label="{{__('Alternative')}} - {{$loop->iteration}}"
                                                 name="questions.{{$loop->parent->index}}.alternatives.{{$loop->index}}.answer"/>
                            </td>
                            <td>
                              {{--                              @include('components.form.CheckBoxInput', ['name' => "questions.".$loop->parent->index.".alternatives.$loop->index.correct", 'label' => '', 'col' => 1])--}}
                              <x-checkbox-field label=""
                                                name="questions.{{$loop->parent->index}}.alternatives.{{$loop->index}}.correct"
                                                required="false"/>
                            </td>
                            <td>
                              <x-confirm-button clickAction="removeAlternative({{$alt->id}})" showConfirmDlg="1"
                                                showPrompt="0"/>
                            </td>
